...
public class CavemansStatsView
{
	...
	public CavemansStatsView( CavemanModel caveman, int tileW, int tileH, TextureAtlas spriteAtlas, TextureAtlas mapAtlas )
	{
	...
	public void renderInventory()
	{
		...
		//y start
		int y = Gdx.graphics.getHeight() - 5 - (int)(itemShed.getRegionHeight() * MVCsManager.tileScale);
		int x = Gdx.graphics.getWidth() / 2 - (int)(itemShed.getRegionWidth() * MVCsManager.tileScale) / 2;
		MVCsManager.spriteBatch.draw( itemShed, x, y, (int)(itemShed.getRegionWidth() * MVCsManager.tileScale),
				(int)(itemShed.getRegionHeight() * MVCsManager.tileScale) );
		MVCsManager.spriteBatch.setColor( color );

		int tmpX = (int)(x + 5 * MVCsManager.tileScale);
		int tmpY = (int)(y + 5 * MVCsManager.tileScale);
		for( Weapon weapon : this.caveman.getWeapons() )
		{
			...
			MVCsManager.spriteBatch.draw( imageRegion, tmpX, tmpY,
					(int)(imageRegion.getRegionWidth() * MVCsManager.tileScale),
					(int)(imageRegion.getRegionHeight() * MVCsManager.tileScale) );
			tmpX += (imageRegion.getRegionWidth() + 1) * MVCsManager.tileScale;
		}
